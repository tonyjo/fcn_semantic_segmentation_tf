...
class Network(object):
                ...
                for param_name, data in data_dict[op_name].iteritems():
                    try:
                        if str(op_name) == 'fc6' and param_name == 'weights':
                            data = np.reshape(data, (7, 7, 512, 4096))
                        elif  str(op_name) == 'fc7' and param_name == 'weights':
                            data = np.reshape(data, (1, 1, 4096, 4096))
                        else:
                            data = data
                        ...
                        if not ignore_missing:
                            print('For layer: ', op_name,', Value not found, normal_init applied!')
    ...
    def dropout(self, input, name):
        output = tf.nn.dropout(input, self.keep_prob, name=name)
        return output
