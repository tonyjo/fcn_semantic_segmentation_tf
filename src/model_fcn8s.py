...
# Init values
bilinear_wts_1  = get_upsampling_weight(in_channels=21, out_channels=21, kernel_size=4)
bilinear_wts_2  = get_upsampling_weight(in_channels=21, out_channels=21, kernel_size=4)
bilinear_wts_3  = get_upsampling_weight(in_channels=21, out_channels=21, kernel_size=16)
bilinear_init_1 = tf.constant_initializer(value=bilinear_wts_1, dtype=tf.float32)
bilinear_init_2 = tf.constant_initializer(value=bilinear_wts_2, dtype=tf.float32)
bilinear_init_3 = tf.constant_initializer(value=bilinear_wts_3, dtype=tf.float32)
...
class FCN8s(object):
    ...
    def build_train_graph(self, l_rate_decay_step, re_use=False):
        ...
        pool_3  = vgg_net.layers['VGG/pool3']
        pool_4  = vgg_net.layers['VGG/pool4']
        ...
        # Upscore-4
        with tf.variable_scope('upscore_4'):
            upscore_4 = slim.conv2d_transpose(fuse_pool4, opt.num_classes, [4, 4],
                                  stride=2, activation_fn=None, padding='VALID',
        ...
        with tf.variable_scope('score_pool3'):
            score_pool3 = slim.conv2d(pool_3, opt.num_classes, [1, 1], stride=1,
                                      activation_fn=None, padding='VALID',
                                      weights_initializer=tf.zeros_initializer(),
                                      biases_initializer=tf.zeros_initializer(),
                                      trainable=True, scope=None)
